...
export class RootService {
  async handleJoin(body) {
    ...
    if (!conditional.phoneCon) {
      return { error: "전화번호 양식이 유효하지 않습니다." };
    }
    const existsName = await User.exists({ name }); // 사용자 이름 존재 확인
    const existsEmail = await User.exists({ email }); // 이메일 존재 확인
    const existsPhone = await User.exists({ phone }); // 전화번호 존재 확인
    try {
      await User.create({ name, email, password, phone }); // 새 사용자 생성
